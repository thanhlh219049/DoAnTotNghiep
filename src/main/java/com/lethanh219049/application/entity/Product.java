...
import com.lethanh219049.application.model.dto.ChartDTO;
...
@SqlResultSetMappings(value = {
                ...
                @SqlResultSetMapping(name = "chartProductDTO", classes = @ConstructorResult(targetClass = ChartDTO.class, columns = {
...
@NamedNativeQuery(name = "searchProductByKeyword", resultSetMapping = "productInfoDto", query = "SELECT DISTINCT p.id, p.name, p.slug, p.sale_price as price, p.product_view as views, p.total_sold, JSON_VALUE(p.images ,'$[0]')  AS images "
                ...
                "LIMIT :limit " +
                "OFFSET :offset ")
@NamedNativeQuery(name = "getProductOrders", resultSetMapping = "chartProductDTO", query = "select p.name as label, sum(o.quantity) as value from product p "
                +
